...
# Output dimensionality of the sentence-transformers "all-MiniLM-L6-v2" model,
# used by Day 4's embedding pipeline (app/services/embedding_service.py).
EMBEDDING_DIM = 384
...
class Intern(Base):
    ...
    # --- Day 4: Embeddings & Skill Matrix ------------------------------
    # Sentence-transformer (all-MiniLM-L6-v2, 384-dim) embedding of this
    # intern's skill/interest profile. Stored as JSON (portable across
    # Postgres/JSONB and other SQLAlchemy-supported backends used in tests)
    # rather than a Postgres-only ARRAY/vector type, keeping the column
    # readable with plain SQL tooling without requiring the `pgvector`
    # extension. Populated by app/services/embedding_service.py.
    skill_embedding = Column(JSON, nullable=True)
    embedding_updated_at = Column(DateTime, nullable=True)
    # SHA-256 of the text used to generate skill_embedding — lets the
    # embedding update service skip recomputation when nothing relevant
    # (skills, tech stack, interests) has actually changed.
    embedding_source_hash = Column(String(64), nullable=True)
