...
— re-themed (lib/theme.py) and gained a "Skill Network" section, the
first frontend surface for the Engineering Knowledge Graph
(app/services/knowledge_graph_service.py), which previously only existed
as raw API endpoints.
...
from lib.theme import apply_theme, render_skill_network
...
st.caption("Cross-Team Analytics · Project Success Rates · Technology Distribution · Skill Network · Resource Utilization")
...
# ---------------------------------------------------------------------------
# Skill Network (post-Day-20 — first frontend surface for the Engineering
# Knowledge Graph, app/services/knowledge_graph_service.py). Renders one
# skill's bipartite subgraph (that skill node <-> every intern who has it)
# via GET /knowledge-graph/skill/{name}/interns — deliberately one skill
# at a time rather than the whole-org graph, which would be an unreadable
# hairball at any real dataset size.
# ---------------------------------------------------------------------------

st.subheader("Skill Network")
st.caption(
    "Who's connected to a given skill, and how strongly — pulled from the "
    "Engineering Knowledge Graph (NetworkX, in-process), not the skill-matrix "
    "table above. Pick a skill to see it as a graph."
)

if ok and tech_data.get("frequency"):
    skill_options = sorted(tech_data["frequency"].keys())
    picked_skill = st.selectbox("Skill", skill_options, key="skill_network_choice")

    with loading(f"Building the graph for {picked_skill}..."):
        ok, skill_interns = get_json(f"/knowledge-graph/skill/{picked_skill}/interns")
        interns_ok, all_interns = get_json("/interns")

    if not ok:
        fetch_error(skill_interns)
    elif not skill_interns:
        empty_state(f"No one in the graph has {picked_skill} yet.")
    else:
        name_by_id = {i["id"]: i["full_name"] for i in all_interns} if interns_ok else {}

        graph_nodes = [{"id": f"skill:{picked_skill}", "label": picked_skill, "kind": "skill"}]
        graph_edges = []
        for entry in skill_interns:
            node_id = f"intern:{entry['intern_id']}"
            label = name_by_id.get(entry["intern_id"], f"#{entry['intern_id']}")
            graph_nodes.append({"id": node_id, "label": f"{label} ({entry['proficiency']}/5)", "kind": "intern"})
            graph_edges.append((node_id, f"skill:{picked_skill}"))

        render_skill_network(graph_nodes, graph_edges, title=f"{picked_skill} — {len(skill_interns)} intern(s)")
elif not ok:
    fetch_error(tech_data)
else:
    empty_state("No skill/technology data yet — import or create some interns first.")
