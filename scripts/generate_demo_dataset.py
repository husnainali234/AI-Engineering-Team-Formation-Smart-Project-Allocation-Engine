...
- **Two interns (ids 21, 22) are marked `is_available=False`** up front —
  so a rebalance demo doesn't need a manual `PUT /interns/{id}` step to
  create the "unavailable member" condition. **Correction from a live
  end-to-end run (post-Day-20 QA):** this alone is *not* enough to make
  `GET /rebalance/needed` show something right after a *default*
  `POST /recommend-teams` call — `/recommend-teams`'s default candidate
  pool is `list_available_unassigned_with_embeddings()`, which excludes
  unavailable interns outright, so 21 and 22 never get placed on a team
  by the default call in the first place. Confirmed live: either (a)
  call `/recommend-teams` with an explicit `intern_ids` list that
  includes 21 and/or 22 (bypasses the default-pool filter), or (b) add
  either of them to an already-formed team via
  `POST /teams/{id}/members`. Either way, `GET /rebalance/needed`
  correctly lists that team afterward — verified against a real running
  instance, not just the test suite.
...
def main():
    ...
    try:
        ...
        print("Demo script: POST /team-formation/preview or "
              "/recommend-teams (pass intern_ids including 21 and/or 22 "
              "if you want the rebalance demo to have something queued "
              "up immediately — the default candidate pool excludes "
              "unavailable interns), then GET /rebalance/needed and "
              "GET /team-chemistry/team/{id} on the two hand-tuned teams "
              "to see the leadership_balance flag differ.")
    finally:
        db.close()
